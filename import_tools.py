...
class DataImporter(HasTraits):
    ...
    def organize_results_by_folder(self, results_list):
        ...
        for result in results_list:
            ...
            if folder not in results_dict.keys():
                results_dict[folder] = {}
            tag_ref = ['signal', 'bg', 'ref']
            for internal_name,tag in zip(tag_ref,tags):
                if tag in name:
                    ...
                    results_dict[folder][striped_name][internal_name] = result
    ...
    def create_measurement_from_data(self,experiment,meas_name,extracted_data):
        ...
        if 'signal' in datasets and 'bg' in datasets:
            datasets['bg_corrected'] = datasets['signal']-datasets['bg']
        cols = []
        for col in ['em_wl','bg_corrected','signal','bg','ref']:
            if col in datasets:
                cols.append(col)
        new_meas.data = datasets[cols]
        return True
...
class ImportTab(HasTraits):
    ...
    view = View(
        ...
        Item(name='start',show_label=False,springy=True,
             enabled_when='len(path_list)',visible_when='not working'),
        ...
        VGroup(
            Group(Item(name='ndone',editor=RangeEditor(low_name='min',
                                                       high_name='ntodo',
                                                       is_float=False,
                                                       ),enabled_when='False',
              show_label=False ),
                  show_border=True,label='Finished'),
